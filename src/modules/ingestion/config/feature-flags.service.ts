...
export class FeatureFlagsService {
  ...
  isFeatureEnabled(
    ...
    if (
      environment &&
      flag.environmentRestrictions &&
      flag.environmentRestrictions.length > 0
    ) {
  ...
  getPlanCapabilities(planName: string): {
    ...
    return {
      ...
      canValidateEmails: this.isFeatureEnabled(
        'advanced_email_validation',
        planName,
      ),
      canResolveBusinessNames: this.isFeatureEnabled(
        'business_name_resolution',
        planName,
      ),
