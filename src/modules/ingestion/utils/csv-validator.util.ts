...
export class CsvValidatorUtil {
  ...
  static validateCsvContent(content: string): CsvValidationResult {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (row.length !== headers.length) {
        errors.push(
          `Row ${i + 2}: Column count mismatch (expected ${headers.length}, got ${row.length})`,
        );
      }
    }
  ...
  static detectEncodingIssues(content: string): string[] {
    ...
    if (content.includes('�')) {
      issues.push(
        'File may have encoding issues (replacement characters detected)',
      );
    }
    ...
    if (
      (hasWindowsEndings && hasUnixEndings) ||
      (hasWindowsEndings && hasMacEndings)
    ) {
