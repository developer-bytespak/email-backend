...
export class SmsService {
  ...
  /**
   * Delete a pending SMS verification (before ClientSms is created)
   */
  async deletePendingSmsVerification(clientId: number, verificationId: number): Promise<void> {
    const scrapingClient = await this.prisma.getScrapingClient();
    ...
    // Verify it belongs to the client and is for SMS
    ...
    if (verification.senderType !== 'sms') {
      throw new BadRequestException('This verification is not for a phone number');
    }

    // Only allow deletion of pending, expired, or rejected verifications
    if (verification.status === 'verified') {
      throw new BadRequestException('Cannot delete verified phone number. Use the delete SMS endpoint instead.');
    }
    ...
    this.logger.log(`✅ Deleted pending SMS verification ${verificationId} for client ${clientId}`);
  }
